...
void main() {
    ...
    float ffx = fff(uv*vec2(3.,3.)*13.2, seed+55.2214);
    float ffy = fff(uv*vec2(3.,3.)*13.2, seed+123.651);

    ffx = .3 + .7*smoothstep(.2, .5, ffx);
    ffy = .3 + .7*smoothstep(.2, .5, ffy);
	...
	//vec3 blur1 = gaussianBlur(tex0, uv, texelSize * vec2(ffx, ffy*.36) * .1*amp*10. + 0.*texelSize * vec2(ffx, ffy) * amp*3. * (3.*pow(uv.y, 6.)));
	//vec3 blur2 = gaussianBlur(tex0, uv, texelSize * vec2(ffx, ffy*.36) * .1*amp*4. + 0.*texelSize * vec2(ffx, ffy) * amp*3. * (3.*pow(uv.y, 6.)));
